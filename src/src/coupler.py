...
class Modes(object):
    ...
    def wave_indexing(self, i):
        self.u = self.u_vec[0, i]
        self.w = self.w_vec[0, i]
        self.k = self.k_vec[i]
        self.beta = self.beta_vec[0, i]
...
class Coupling_coefficients(object):
    def __init__(self, lmin, lmax, N, a, N_points):
        self.lmin = lmin
        self.lmax = lmax
    ...
    def get_eigenvalues(self):
        ...
        u_01, w_01, neff01 = [np.zeros([1, self.N]) for i in range(3)]
        u_11, w_11, neff11 = [np.zeros([2, self.N]) for i in range(3)]
        for i in range(self.N):
            u_01[0, i], w_01[0, i], neff01[0, i] = e.solve_01(i)
    ...
    def create_coupling_coeff(self, d):
        ...
        couple01 = Coupling_coeff(
            self.a, self.N_points, self.e.ncore,
            self.e.nclad, self.e.l_vec, neff01[0, :])
        couple11 = Coupling_coeff(self.a, self.N_points, self.e.ncore,
                                  self.e.nclad, self.e.l_vec, neff11[0, :])
        couple01.initialise_integrand_vectors(d, Eabs01)
        couple11.initialise_integrand_vectors(d, Eabs11)

        k01 = couple01.f_vec * (pi / (neff01[0, :] * c)) * couple01.integrals()
...
def calculate_coupling_coeff(lmin, lmax, a, N_points, N_l, d_vec):
    ...
    k01_1, k11_1, couple01_1, couple11_1 = couple_obj.create_coupling_coeff(
        d_vec[0])

    k01_2, k11_2, couple01_2, couple11_2 = couple_obj.create_coupling_coeff(
        d_vec[1])

    D = {'k01_1': k01_1, 'k11_1': k11_1, 'k01_2': k01_2,
         'k11_2': k11_2, 'f_meas': couple11_1.f_vec}
    save_coupling_coeff(**D)
    return k01_1, k11_1, couple01_1, couple11_1, \
        k01_2, k11_2, couple01_2, couple11_2


def save_coupling_coeff(filename = 'coupling_coeff', filepath='loading_data/', **variables):
    full_file = filepath + filename + '.hdf5'

    if os.path.isfile(full_file):
        os.system('rm '+full_file)
    with h5py.File(full_file, 'a') as f:
        for i in (variables):
